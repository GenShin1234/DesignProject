    ...
    while retry_count < max_retries and SC_Num > 0 and total_num < max_retries * SC_Num:
        ...
        for response in responses:
            try:
                ...
                text_query, id_query, step_query = sllm.align.MetaQA_text2query(args, response, question, table_data,
                                                                                relations)  # text_query:进行node2id前的query，id_query:进行node2id后的query
                ...
                if target_type == None:
                    res, mid_output = table_data.excute_query(args, id_query, target_type=None, node_query=text_query,
                                                              task=step_query, question=question)
                else:
                    res, mid_output = table_data.excute_query(args, id_query, target_type=target_type[0],
                                                              node_query=text_query, task=step_query, question=question)
            ...
            if res == None or res == [] or res == set() or res == [set()] or res == dict() or res == [
                set([])] or res == ['None'] or res == ['none'] or (type(res) == str and "[line_" in res):
