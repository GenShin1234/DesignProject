...
class CronQuestionDataFormat:
    def __init__(self, question: str, relation_list: list, annotation: dict, answer: list = None,
                 answer_type: str = None, _type: str = None):
...
def read_tempKG(args):
    ...
    KG_data = sllm.cg.data(test_table_['triples'], test_table_['entities_2_line'], test_table_['all_lines_id'],
                           if_temp=True)
...
def KG2CG(folder_path):
    ...
    for path_of_file in tqdm(paths_of_files):
        ...
        KG_data[table_name] = sllm.cg.data(test_table_['triples'], test_table_['entities_2_line'],
                                           test_table_['all_lines_id'])
...
def get_wqsp_question(data_path):
    ...
    with open(data_path, 'r') as fp:
        for line in fp:
            ...
            KGQA_data.append(
                WQSPDataFormat(question, TopicEntityName, First_step, Second_step, TopicEntityID, table_id, answer))
